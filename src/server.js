...
// Sentry gateway configuration
const SENTRY_GATEWAY_PORT = Number.parseInt(process.env.SENTRY_GATEWAY_PORT ?? "18790", 10);
const SENTRY_GATEWAY_HOST = process.env.SENTRY_GATEWAY_HOST ?? "127.0.0.1";
const SENTRY_GATEWAY_TARGET = `http://${SENTRY_GATEWAY_HOST}:${SENTRY_GATEWAY_PORT}`;
...
// Sentry gateway process
let sentryGatewayProc = null;
let sentryGatewayStarting = null;
...
// Sentry gateway management functions
async function startSentryGateway() {
  if (sentryGatewayProc) return;
  
  console.log("[sentry] Starting Sentry gateway...");
  
  // Use bun to run the Sentry gateway
  const bunPath = "/root/.bun/bin/bun"; // Bun installed in build stage
  const sentryArgs = ["--cwd", "gateway", "run", "start"];
  
  sentryGatewayProc = childProcess.spawn(bunPath, sentryArgs, {
    stdio: "inherit",
    env: {
      ...process.env,
      PORT: String(SENTRY_GATEWAY_PORT),
    },
  });

  sentryGatewayProc.on("error", (err) => {
    const msg = `[sentry] spawn error: ${String(err)}`;
    console.error(msg);
    sentryGatewayProc = null;
  });

  sentryGatewayProc.on("exit", (code, signal) => {
    const msg = `[sentry] exited code=${code} signal=${signal}`;
    console.error(msg);
    sentryGatewayProc = null;
  });
  
  console.log(`[sentry] Sentry gateway starting on port ${SENTRY_GATEWAY_PORT}`);
}

async function ensureSentryGatewayRunning() {
  if (sentryGatewayProc) return { ok: true };
  if (!sentryGatewayStarting) {
    sentryGatewayStarting = (async () => {
      try {
        await startSentryGateway();
        // Wait for Sentry to start
        await sleep(3000);
      } catch (err) {
        const msg = `[sentry] start failure: ${String(err)}`;
        console.error(msg);
        throw err;
      }
    })().finally(() => {
      sentryGatewayStarting = null;
    });
  }
  await sentryGatewayStarting;
  return { ok: true };
}

async function probeSentryGateway() {
  const net = await import("node:net");
  
  return await new Promise((resolve) => {
    const sock = net.createConnection({
      host: SENTRY_GATEWAY_HOST,
      port: SENTRY_GATEWAY_PORT,
      timeout: 750,
    });
...
const sentryProxy = httpProxy.createProxyServer({
  target: SENTRY_GATEWAY_TARGET,
  ws: true,
  xfwd: true,
});

sentryProxy.on("error", (err, _req, res) => {
  console.error("[sentry-proxy]", err);
  try {
    if (res && typeof res.writeHead === "function" && !res.headersSent) {
      res.writeHead(502, { "Content-Type": "text/plain" });
      res.end("Sentry Gateway unavailable\n");
    }
  } catch {
    // ignore
  }
...
// Sentry gateway route - proxy to Sentry instead of OpenClaw
app.use("/sentry", async (req, res) => {
  try {
    await ensureSentryGatewayRunning();
  } catch (err) {
    return res.status(503).type("text/plain").send(`Sentry Gateway not ready: ${err}`);
  }
  return sentryProxy.web(req, res, { target: SENTRY_GATEWAY_TARGET });
});
...
const server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  // Start Sentry gateway
  console.log("[sentry] Starting Sentry gateway...");
  try {
    await ensureSentryGatewayRunning();
    console.log("[sentry] Sentry gateway ready");
  } catch (err) {
    console.error(`[sentry] Sentry gateway failed to start: ${String(err)}`);
  }
});
...
process.on("SIGTERM", () => {
  ...
  // Stop Sentry gateway
  try {
    if (sentryGatewayProc) sentryGatewayProc.kill("SIGTERM");
  } catch {
    // ignore
  }
